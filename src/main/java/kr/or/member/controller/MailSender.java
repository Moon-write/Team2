...
public class MailSender {
	public String sendMail(String email) {
		...
		try {
			...
			//보내는 사람 정보 설정
			msg.setFrom(new InternetAddress("hakwon789456@gmail.com","WeNeedYou"));
			//받는사람 정보 설정
			InternetAddress to = new InternetAddress(email);
			msg.setRecipient(Message.RecipientType.TO, to);
			//이메일 제목설정
			msg.setSubject("WeNeedYou 인증메일입니다.","UTF-8");
			//이메일 본문설정
			msg.setContent("<h1>안녕하세요. WeNeedYou 입니다</h1>"
						+ "<h3>회원님의 인증번호는 [<span style='color:red;'>"+sb.toString()+"</span>]입니다.</h3>"
						+ "<h3>회원님께 만족을 드리는 WeNeedYou가 되겠습니다. 감사합니다</h3>"
